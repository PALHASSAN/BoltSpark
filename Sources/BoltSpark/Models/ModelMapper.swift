...
public class ModelMapper {
    ...
    public static func map<T: Model>(_ rows: [[String: Any]], to type: T.Type) throws -> [T] {
        ...
        if relationsCache[typeName] == nil {
            ...
            if let template = try? decoder.decode(T.self, from: "{}".data(using: .utf8)!) {
        ...
        let processedRows = rows.map { row -> [String: Any] in
            var newRow = row
            relationKeys.forEach { newRow[$0] = NSNull() }
            return newRow
        }

        let jsonData = try JSONSerialization.data(withJSONObject: processedRows, options: [])
        let finalDecoder = JSONDecoder()
        finalDecoder.keyDecodingStrategy = .convertFromSnakeCase
        
        do {
            return try finalDecoder.decode([T].self, from: jsonData)
        } catch {
            throw BoltError.mappingError("🧩 BoltSpark Mapping Failed: \(T.self)")
        }
    }
}
